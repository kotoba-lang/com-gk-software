(ns gk_software.main
  "Kotodama WASM entrypoint for the gk_software clean-room actor (L4) — Clojure port.
  ...
  facts (`gk_software.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "gk_software")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "gksoftwa_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "gksoftwa_cus"
    :fields [:email :firstName :lastName :phone]       :required [:email :firstName]
    :coerce {}                               :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "gksoftwa_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"gk_software.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gk_software-compat" :tier tier :entities entities} 200])
...
(defn start! [] :gk_software-compat/ready)
